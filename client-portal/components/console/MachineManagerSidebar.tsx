...
const navLinks: NavLink[] = [
  ...
  {
    href: '/console/machine-manager/machines',
    label: 'My Machines',
    icon: <Server className="h-4 w-4" />,
  },
  {
    href: '/console/machine-manager/jobs',
    label: 'Jobs & Status',
    icon: <Briefcase className="h-4 w-4" />,
  },
];

interface MachineManagerSidebarProps {
  sidebarOpen: boolean;
  onCloseSidebar: () => void;
}

export function MachineManagerSidebar({
  sidebarOpen,
  onCloseSidebar,
}: MachineManagerSidebarProps) {
      ...
      >
        <div className="flex h-full min-w-[15rem] flex-col">
          <div className="border-b border-gray-100 px-5 py-5">
            ...
            <p className="mt-0.5 text-xs text-gray-400">Install &amp; manage software</p>
          </div>
          ...
          <nav className="flex-1 space-y-0.5 overflow-y-auto p-3">
            {navLinks.map((link) => {
              const isActive =
                pathname === link.href ||
                (link.href !== '/console/machine-manager' &&
