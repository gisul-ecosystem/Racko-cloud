...
import { deleteTenantUser, fetchTenantUsers, setTenantUserActive } from '@/lib/tenantVmApi';
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-gray-500">Shared tenant users for VPS and Elastic Server Import</p>
        </div>
        <Link
                    ...
                    return (
                      <tr key={user.id} className="border-b border-gray-50">
                        ...
                        <td className="px-4 py-3">
                          <span className={`inline-flex rounded-full px-2 py-0.5 text-xs font-medium ${user.isActive ? 'bg-green-50 text-green-700' : 'bg-gray-100 text-gray-600'}`}>
                            {user.isActive ? 'Active' : 'Inactive'}
                          </span>
                        </td>
                        <td className="px-4 py-3 text-gray-600">
                          {serverCount > 0 ? `${serverCount} server${serverCount === 1 ? '' : 's'}` : <span className="text-gray-400">None</span>}
                        </td>
                        <td className="px-4 py-3 text-xs text-gray-500">{new Date(user.createdAt).toLocaleDateString()}</td>
                        <td className="px-4 py-3">
                          <div className="flex flex-wrap gap-2">
                            <button type="button" disabled={actionId === user.id} onClick={() => void handleToggleActive(user)} className="rounded-lg border border-gray-200 px-3 py-1.5 text-xs">
                              {user.isActive ? 'Deactivate' : 'Activate'}
                            </button>
                            <button type="button" disabled={actionId === user.id} onClick={() => setDeleteTarget(user)} className="inline-flex items-center gap-1 rounded-lg border border-red-200 px-3 py-1.5 text-xs text-red-700">
