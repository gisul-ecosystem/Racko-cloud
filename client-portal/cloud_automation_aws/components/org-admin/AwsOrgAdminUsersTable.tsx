...
function DailyUsageBar({ usage }: { usage: AwsOrgAdminDailyUsageEntry }) {
...
function TimeSpentCell({ user }: { user: AwsOrgAdminUser }) {
...
export function AwsOrgAdminUsersTable({
  detail,
  iamPolicies,
  saving,
  onSuspend,
  ...
  const [costData, setCostData] = useState<AwsOrgAdminUserCost | null>(null);
  ...
  useEffect(() => {
    ...
    const fetchDailyUsage = async () => {
      try {
        const response = await getAwsOrgDailyUsage(detail.requestId);
        if (response.success && response.data) {
          const map: Record<number, AwsOrgAdminDailyUsageEntry> = {};
  ...
  return (
    <>
      <p className="mb-4 text-xs text-gray-500">
        Manage IAM lab users, permissions, and AWS console access.
        {showUsageTracking
            ...
            {pageUsers.map((user) => {
              ...
              return (
                <tr key={user.userIndex} className="border-b border-gray-50">
                  ...
                  <td className="px-4 py-3">
                    <AwsLabStatusBadge status={user.status} />
                  </td>
                  ...
                  <td className="px-4 py-3">
                    {detail.accessType === 'identity_center' ? (
