...
export function AwsOrgAdminAddUsersModal({
  usersCount,
  request,
  submitting,
  onClose,
  ...
  usersCount: number;
  request: AwsOrgAdminRequestDetail | null;
  submitting: boolean;
  onClose: () => void;
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
      <div
        ...
        role="dialog"
        aria-labelledby="aws-add-users-title"
      >
        <div className="flex items-start justify-between border-b border-gray-100 px-5 py-4">
          <div>
            <h3 id="aws-add-users-title" className="text-base font-semibold text-gray-900">
              Add users
            </h3>
            <p className="mt-0.5 text-sm text-gray-500">
              Provision AWS lab users and email credentials to the customer.
            </p>
          </div>
          <button
            type="button"
        ...
        <form onSubmit={(event) => void handleSubmit(event)} className="space-y-4 px-5 py-4">
          <div>
            <label htmlFor="aws-add-users-count" className="block text-sm font-medium text-gray-700">
              Account count
            </label>
            <input
              id="aws-add-users-count"
              type="number"
              min={1}
