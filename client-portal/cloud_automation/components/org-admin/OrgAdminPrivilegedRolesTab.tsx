...
import {
  assignOrgPrivilegedRoleToAllUsers,
  listOrgAzureRoles,
  listOrgPrivilegedRoleRequests,
  reviewOrgPrivilegedRoleRequest,
} from '../../api/orgAdminClient';
import type { OrgAdminPrivilegedRoleRequest, OrgAdminUser } from '../../types/orgAdmin';
...
interface OrgAdminPrivilegedRolesTabProps {
  requestId: number;
  users: OrgAdminUser[];
  onAssigned?: () => void;
}
...
export function OrgAdminPrivilegedRolesTab({
  requestId,
  users,
  onAssigned,
}: OrgAdminPrivilegedRolesTabProps) {
  const [privilegedRoles, setPrivilegedRoles] = useState<{ name: string; definitionId: string }[]>(
    []
  );
  const [pendingRequests, setPendingRequests] = useState<OrgAdminPrivilegedRoleRequest[]>([]);
  ...
  const loadData = useCallback(async () => {
    try {
      const [roles, pending] = await Promise.all([
        listOrgAzureRoles(),
        listOrgPrivilegedRoleRequests({ status: 'pending', requestId }),
      ]);

      const filteredRoles = roles.filter((role) =>
        PRIVILEGED_ROLE_NAMES.has(role.name.toLowerCase())
      );
      setPrivilegedRoles(filteredRoles);
      setPendingRequests(pending);
      setSelectedRole((current) => current || filteredRoles[0]?.name || '');
    } catch (error) {
  ...
  async function handleAssignAll() {
    ...
    setAssigning(true);
    try {
      const result = await assignOrgPrivilegedRoleToAllUsers(requestId, selectedRole);
  ...
  async function handleReview(id: number, status: 'approved' | 'rejected') {
    setReviewingId(id);
    try {
      await reviewOrgPrivilegedRoleRequest(id, { status });
      showBanner(
        'success',
        status === 'approved'
          ? 'Privileged role request approved and assigned to all users.'
    ...
    <div className="space-y-6 p-5">
        ...
        <div className="rounded-xl border border-violet-200 bg-violet-50/40 p-4">
          ...
          <ul className="space-y-3">
            {pendingRequests.map((request) => (
              ...
              >
                <div>
                  <p className="text-sm font-medium text-gray-900">{request.azureRole}</p>
      ...
      <div className="rounded-xl border border-gray-200 bg-white p-4 shadow-sm">
        ...
        <p className="mt-1 text-xs text-gray-500">
          Assign a built-in privileged Azure role to all {eligibleUsers.length} provisioned user(s)
          in this lab. Owner is not available.
        </p>

        <div className="mt-4 flex flex-col gap-3 sm:flex-row sm:items-end">
          <div className="flex-1">
            <label
              htmlFor="privileged-role-select"
              className="mb-1.5 block text-xs font-medium uppercase tracking-wide text-gray-500"
            >
              Privileged role
            </label>
            <select
              id="privileged-role-select"
              value={selectedRole}
