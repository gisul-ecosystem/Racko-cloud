...
import type { OrgAdminAccessRequest } from '../../types/orgAdmin';

interface OrgAdminAccessRequestsProps {
  requests: OrgAdminAccessRequest[];
  loading: boolean;
  saving: boolean;
  onReview: (
    id: number,
...
export function OrgAdminAccessRequests({
  requests,
  loading,
  saving,
  onReview,
}: OrgAdminAccessRequestsProps) {
    ...
    <div className="rounded-xl border border-amber-200 bg-amber-50/50 shadow-sm">
      <div className="flex items-center gap-2 border-b border-amber-100 px-5 py-3">
        ...
        <h2 className="text-sm font-semibold text-amber-900">
          Pending admin access requests ({requests.length})
        </h2>
      </div>
          ...
          return (
            <li key={request.id} className="px-5 py-4">
              ...
              <input
                ...
                className="mt-3 w-full rounded-lg border border-amber-100 bg-white px-3 py-2 text-xs text-gray-900 focus:border-[#B91C1C] focus:outline-none focus:ring-1 focus:ring-[#B91C1C]"
              />
            </li>
          );
        })}
