...
import {
  assignGcpOrgPrivilegedRoleToAllUsers,
  listGcpOrgPrivilegedRoleRequests,
  listGcpOrgPrivilegedRoles,
...
interface GcpOrgAdminPrivilegedRolesTabProps {
  requestId: string;
...
export function GcpOrgAdminPrivilegedRolesTab({
  requestId,
  users,
  onAssigned,
}: GcpOrgAdminPrivilegedRolesTabProps) {
  const [privilegedRoles, setPrivilegedRoles] = useState<GcpPrivilegedRoleOption[]>([]);
  const [pendingRequests, setPendingRequests] = useState<GcpOrgAdminPrivilegedRoleRequest[]>([]);
  ...
  const loadData = useCallback(async () => {
    try {
      const [roles, pending] = await Promise.all([
        listGcpOrgPrivilegedRoles(),
        listGcpOrgPrivilegedRoleRequests({ status: 'pending', requestId }),
      ]);
  ...
  async function handleAssignAll() {
    ...
    setAssigning(true);
    try {
      const result = await assignGcpOrgPrivilegedRoleToAllUsers(requestId, selectedRole);
  ...
  async function handleReview(id: string, status: 'approved' | 'rejected') {
    setReviewingId(id);
    try {
      await reviewGcpOrgPrivilegedRoleRequest(id, { status });
      showBanner(
        'success',
        status === 'approved'
        ...
        <div className="rounded-xl border border-violet-200 bg-violet-50/40 p-4">
          ...
          <ul className="space-y-3">
            {pendingRequests.map((request) => (
              ...
              >
                <div>
                  <p className="text-sm font-medium text-gray-900">{request.gcpRole}</p>
