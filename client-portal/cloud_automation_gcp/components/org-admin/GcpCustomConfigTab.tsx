...
import {
  addGcpCustomServiceToRequest,
  assignGcpCustomIamPolicy,
  assignGcpCustomIamPolicyToAll,
  createGcpCustomIamPolicy,
  createGcpCustomService,
  deleteGcpCustomIamPolicy,
  deleteGcpCustomService,
  listGcpCustomIamPolicies,
  listGcpCustomIamAssignments,
  listGcpCustomServices,
  listGcpRequestCustomServices,
  removeGcpCustomServiceFromRequest,
  revokeGcpCustomIamAssignment,
  updateGcpCustomIamPolicy,
  updateGcpCustomService,
...
export function GcpCustomConfigTab({
  requestId,
  users,
}: {
  requestId: string;
  ...
  const [policies, setPolicies] = useState<GcpCustomIamPolicy[]>([]);
  const [services, setServices] = useState<GcpCustomService[]>([]);
  const [requestServices, setRequestServices] = useState<GcpCustomService[]>([]);
  const [assignments, setAssignments] = useState<GcpCustomIamPolicyAssignment[]>([]);
  ...
  const load = useCallback(async () => {
    try {
      const [nextPolicies, nextServices, selected, nextAssignments] = await Promise.all([
        listGcpCustomIamPolicies(),
        listGcpCustomServices(),
        listGcpRequestCustomServices(requestId),
        listGcpCustomIamAssignments(requestId),
      ]);
  ...
  async function createPolicy() {
    ...
    await act(
      () => createGcpCustomIamPolicy({ name: policyName.trim(), description: policyDescription.trim(), document }),
  ...
  async function assignPolicy() {
    ...
    await act(
      () =>
        assignmentUser === 'all'
          ? assignGcpCustomIamPolicyToAll(requestId, assignmentPolicyId)
          : assignGcpCustomIamPolicy(requestId, Number(assignmentUser), assignmentPolicyId),
    ...
    <div className="divide-y">
      ...
      <section className="space-y-4 px-6 py-5">
        <div>
          ...
          <p className="text-sm text-gray-500">Define reusable Gcp policy documents for lab users.</p>
        </div>
          ...
          <div key={policy.id} className="flex items-start justify-between gap-3 rounded-lg border p-4">
            ...
            <div className="flex gap-1">
              <button type="button" disabled={busy} onClick={() => {
                ...
                if (name) void act(() => updateGcpCustomIamPolicy(policy.id, { name }), 'Policy updated.');
              }} className="rounded border p-2 text-gray-600"><Pencil className="h-3.5 w-3.5" /></button>
              <button type="button" disabled={busy} onClick={() => window.confirm(`Delete "${policy.name}"?`) && void act(() => deleteGcpCustomIamPolicy(policy.id), 'Policy deleted.')} className="rounded border border-red-200 p-2 text-red-600"><Trash2 className="h-3.5 w-3.5" /></button>
            </div>
          </div>
        ))}
              ...
              {assignments.map((assignment) => {
                ...
                return (
                  <div key={assignmentId} className="flex items-center justify-between gap-3 px-3 py-2 text-sm">
                    ...
                    <button type="button" disabled={busy} onClick={() => void act(() => revokeGcpCustomIamAssignment(assignmentId), 'Policy assignment revoked.')} className="text-xs font-medium text-red-700">Revoke</button>
                  </div>
                );
              })}
            </div>
        ...
        {services.length === 0 ? <p className="rounded-lg border border-dashed p-8 text-center text-sm text-gray-500">No custom services.</p> : services.map((service) => {
          ...
          return <div key={service.id} className="flex flex-wrap items-center justify-between gap-3 rounded-lg border p-4">
            ...
            <div className="flex gap-2">
              <button type="button" disabled={busy} onClick={() => void act(() => added ? removeGcpCustomServiceFromRequest(requestId, service.id) : addGcpCustomServiceToRequest(requestId, service.id), added ? 'Service removed from request.' : 'Service added to request.')} className={`rounded-lg px-3 py-1.5 text-xs font-medium ${added ? 'border bg-green-50 text-green-800' : 'bg-[#B91C1C] text-white'}`}>{added ? '✓ Added' : 'Add to request'}</button>
              <button type="button" disabled={busy} onClick={() => {
                const name = window.prompt('Service name', service.name);
                if (name) void act(() => updateGcpCustomService(service.id, { name }), 'Service updated.');
              }} className="rounded border p-2 text-gray-600"><Pencil className="h-3.5 w-3.5" /></button>
              <button type="button" disabled={busy} onClick={() => window.confirm(`Delete "${service.name}"?`) && void act(() => deleteGcpCustomService(service.id), 'Service deleted.')} className="rounded border border-red-200 p-2 text-red-600"><Trash2 className="h-3.5 w-3.5" /></button>
            </div>
          </div>;
        })}
      </section>
