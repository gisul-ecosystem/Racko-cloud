...
function validateStep(step, input) {
  ...
  if (step === 1) {
    ...
    if (!input.idMode) {
      errors.push('Select GCP test_ids or GCP IDs.');
    }
    if (
  ...
  if (step === 9 && !input.region) {
    errors.push('Select an Gcp region.');
  }
...
export function RequestWorkspace() {
  ...
  const [idMode, setIdMode] = useState(isPurchaseConvert ? 'gcp_ids' : null);
  ...
  const GcpIdsSnapshotRef = useRef(null);
  ...
  const [accessType, setAccessType] = useState(() => {
    ...
    return days > 7 ? 'cloud_identity' : 'magic_link';
  });
  ...
  const pricingRegion = region || GCP_DEFAULT_REGION;
  ...
  const handleSubmitPrivilegedRoleRequest = useCallback(async () => {
    ...
    try {
      await createPrivilegedRoleRequest({
        ...
        GcpRole: selectedPrivilegedRole,
      });
  ...
  const handleIdModeChange = useCallback(
    (mode) => {
      ...
      const snapshot = GcpIdsSnapshotRef.current;
      setIdMode('gcp_ids');
  ...
  const handleSubmit = async () => {
    ...
    const payload = {
      ...
      id_mode: isPurchaseConvert ? 'gcp_ids' : idMode,
