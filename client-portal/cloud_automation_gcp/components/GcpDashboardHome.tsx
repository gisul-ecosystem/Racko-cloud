...
export function GcpDashboardHome() {
  ...
  const { requests, loading, error, refetch } = useGcpRequests(isAuthenticated);
  ...
  const stats = useMemo(() => {
    const next = { total: 0, completed: 0, provisioning: 0, failed: 0 };
    for (const request of filteredRequests) {
      next.total += 1;
      const category = categorizeGcpRequestStatus(request.status ?? '');
      if (category === 'completed') next.completed += 1;
      else if (category === 'provisioning') next.provisioning += 1;
      else if (category === 'failed' || category === 'expired') next.failed += 1;
    }
    return next;
  }, [filteredRequests]);
          ...
          <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
              ...
              {loading ? (
                ...
                <div className="px-6 py-14 text-center">
                  ...
                  <p className="mx-auto mt-2 max-w-sm text-sm text-gray-500">
                    {filterProjectId
                      ? 'Create a GCP request from this project’s Use service flow to see it here.'
                      : 'Create your first GCP lab request to provision Cloud Identity users and IAM access.'}
                  </p>
                  <Link
                    href={
                      filterProjectId
                ...
                <div className="overflow-x-auto">
                  <table className="w-full text-sm">
                    ...
                    <tbody className="divide-y divide-gray-50">
                      {recentRequests.map((request: GcpRequest) => {
                        ...
                        return (
                          <tr
                            key={request._id}
                            onClick={() => router.push(routes.requestStatus(request._id))}
                          ...
                          >
                            <td className="px-6 py-3.5 font-mono text-xs font-medium text-gray-600">
                              #{String(request._id).slice(-8)}
                            </td>
                            <td className="px-4 py-3.5">
                              <span
                                className="inline-flex max-w-[10rem] truncate rounded-full bg-gray-100 px-2.5 py-0.5 text-xs font-semibold text-gray-700 ring-1 ring-inset ring-gray-200"
                                title={getProjectName(request)}
                              >
                            ...
                            <td className="px-4 py-3.5 text-gray-600">
                              ...
                              >
                                ...
                                {formatGcpRegion(request.region)}
                              </span>
                            </td>
                            <td className="px-4 py-3.5 text-gray-600">{getAccountCount(request)}</td>
                            <td className="px-4 py-3.5">
                              <GcpRequestStatusBadge status={getRequestStatus(request)} />
                            </td>
            ...
            <div className="space-y-6">
              <div className="rounded-xl border border-gray-200 bg-white p-6 shadow-sm">
                ...
                <p className="mt-0.5 text-xs text-gray-400">Common tasks for GCP automation</p>
              ...
              <div className="rounded-xl border border-gray-200 bg-white p-6 shadow-sm">
                ...
                <ul className="space-y-3 text-sm leading-relaxed text-gray-600">
                  <li className="flex gap-3">
                    ...
                    Lab users are created in Cloud Identity and receive IAM roles on the GCP project.
                  </li>
                  <li className="flex gap-3">
                    <ArrowRight className="mt-0.5 h-4 w-4 shrink-0" style={{ color: accent }} />
                    Completed requests send credentials and a manage portal link to the customer email.
                  </li>
                  <li className="flex gap-3">
                    <ArrowRight className="mt-0.5 h-4 w-4 shrink-0" style={{ color: accent }} />
                    Without billing, labs share the master project until dedicated projects are enabled.
                  </li>
                  <li className="flex gap-3">
                    <ArrowRight className="mt-0.5 h-4 w-4 shrink-0" style={{ color: accent }} />
                    Use Org Admin for elevated access workflows and org-wide lab management.
                  </li>
                </ul>
              </div>
            </div>
