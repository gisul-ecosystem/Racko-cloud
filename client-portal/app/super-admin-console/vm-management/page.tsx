  ...
  if (alerts.length === 0) {
    return (
      <div className="bg-green-50 border border-green-200 rounded-xl p-4 shadow-sm flex items-center justify-between">
        ...
        <Link href="/super-admin-console/vm-management/alerts" className="text-xs text-green-600 hover:text-green-700 font-medium flex items-center gap-1">
  ...
  return (
    <div className={`border rounded-xl p-4 shadow-sm ${critical.length > 0 ? 'bg-red-50 border-red-200' : 'bg-yellow-50 border-yellow-200'}`}>
      <div className="flex items-center justify-between mb-3">
        ...
        <Link href="/super-admin-console/vm-management/alerts" className={`text-xs font-medium flex items-center gap-1 ${critical.length > 0 ? 'text-red-600 hover:text-red-700' : 'text-yellow-600 hover:text-yellow-700'}`}>
...
export default function VmManagementOverviewPage() {
  const { user } = useAuth();
  ...
  return (
    <div className="max-w-screen-xl">
      ...
      {data && (
        <LastUpdated fetchedAt={data.fetchedAt} loading={loading} onRefresh={refetch} />
      )}
