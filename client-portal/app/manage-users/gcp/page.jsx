...
import {
  fetchGcpManagePortalData,
  loginGcpManagePortal,
  syncGcpRequestSpend,
} from '../../../cloud_automation_gcp/api/managePortalClient';
import {
  clearGcpManagePortalSession,
  loadGcpManagePortalSession,
  saveGcpManagePortalSession,
} from '../../../cloud_automation_gcp/utils/gcpManagePortalSession';
import GcpUserAccountView from './components/GcpUserAccountView';
import InfoCards from './components/InfoCards';
import ManagePortalLogin from './components/ManagePortalLogin';
import PortalHeader from './components/PortalHeader';
import UsersTable from './components/UsersTable';

function GcpManagePortalContent() {
  ...
  useEffect(() => {
    setSession(loadGcpManagePortalSession());
  ...
  const loadPortalData = useCallback(async (activeSession, syncSpend = false) => {
    ...
    try {
      if (syncSpend && activeSession.role !== 'user') {
        try {
          await syncGcpRequestSpend(activeSession.requestId, activeSession.jwtToken);
        } catch {
          // Spend sync is best-effort; still load portal data.
        }
      }

      const data = await fetchGcpManagePortalData(
  ...
  const handleLogin = useCallback(
    async (credentials) => {
      ...
      try {
        const result = await loginGcpManagePortal({ token: urlToken, ...credentials });
        ...
        saveGcpManagePortalSession(nextSession);
  ...
  const portalDescription = isUser
    ? 'View your provisioned GCP project status, session activity, and open the GCP Console.'
    : 'Review provisioned GCP users, launch console access, and manage budgets and cleanup.';
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <main className="mx-auto max-w-screen-xl space-y-6 px-4 py-8 sm:px-6 lg:px-8">
        ...
        <InfoCards
          ...
          gcpProjectId={portalData?.gcpProjectId}
...
export default function GcpManageUsersPage() {
  return (
    <Suspense fallback={<ManageUsersFallback />}>
      <GcpManagePortalContent />
    </Suspense>
  );
}
