...
export default function SuperAdminVMsPage() {
  ...
  // Filters sent to backend as query params — server-side filtering
  ...
  // Accumulate known nodes from results so dropdown stays populated when a node filter is active
  useEffect(() => {
    if (vms.length > 0) {
      setKnownNodes((prev) => {
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-0.5">
            {loading ? 'Loading…' : `${filtered.length} of ${vms.length} VMs across all admins`}
          </p>
        </div>
        <button
          onClick={refetch}
