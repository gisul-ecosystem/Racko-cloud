...
function exportToCsv(sessions: TenantConsoleSessionEntry[]) {
...
export default function TenantElasticAnalyticsPage() {
  ...
  const load = useCallback(async () => {
    ...
    setError(null);
    try {
      const result = await fetchTenantConsoleSessions({
  ...
  }, [tenantUser, fromDate, toDate, page]);
  ...
  const filtered = sessions.filter((s) => {
    ...
    const matchSearch = !q || s.userEmail.toLowerCase().includes(q) || s.serverName.toLowerCase().includes(q);
    const matchStatus =
      statusFilter === 'all' ||
    ...
    <div className="max-w-screen-xl">
      ...
      {summary && (
        <div className="mb-6 grid grid-cols-2 gap-4 lg:grid-cols-4">
          <SummaryCard icon={Activity} label="Sessions Today" value={summary.sessionsToday} color="text-blue-600" />
          <SummaryCard icon={Users} label="Active Users Today" value={summary.uniqueUsersToday} color="text-purple-600" />
          <SummaryCard icon={Clock} label="Avg Session Duration" value={formatDuration(summary.avgDurationSeconds)} color="text-amber-600" sub="completed sessions today" />
          <SummaryCard icon={Wifi} label="Currently Live" value={summary.activeSessions} color="text-green-600" sub={summary.activeSessions > 0 ? 'users connected now' : 'no active sessions'} />
        </div>
      )}
      ...
      <div className="mb-4 flex flex-wrap items-center gap-3">
        <div className="relative">
          ...
          <input
            ...
            className="h-9 w-56 rounded-lg border border-gray-200 bg-white pl-8 pr-3 text-sm text-gray-700 placeholder:text-gray-400 focus:border-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-500/20"
          />
        </div>
        <div className="flex items-center gap-1.5">
          <Calendar className="h-3.5 w-3.5 text-gray-400" />
          <input type="date" value={fromDate} onChange={(e) => { setFromDate(e.target.value); setPage(1); }} className="h-9 rounded-lg border border-gray-200 bg-white px-2.5 text-sm text-gray-700 focus:border-blue-500 focus:outline-none" />
          <span className="text-xs text-gray-400">to</span>
          <input type="date" value={toDate} onChange={(e) => { setToDate(e.target.value); setPage(1); }} className="h-9 rounded-lg border border-gray-200 bg-white px-2.5 text-sm text-gray-700 focus:border-blue-500 focus:outline-none" />
        </div>
        <select value={statusFilter} onChange={(e) => setStatusFilter(e.target.value as 'all' | 'active' | 'completed')} className="h-9 rounded-lg border border-gray-200 bg-white px-2.5 text-sm text-gray-700 focus:border-blue-500 focus:outline-none">
          <option value="all">All statuses</option>
          <option value="active">Active</option>
          <option value="completed">Completed</option>
        </select>
        {(fromDate || toDate || search || statusFilter !== 'all') && (
          <button onClick={() => { setSearch(''); setFromDate(''); setToDate(''); setStatusFilter('all'); setPage(1); }} className="text-xs text-gray-400 hover:text-gray-600">
            Clear filters
          </button>
        )}
      </div>
      ...
      <div className="overflow-hidden rounded-xl border border-gray-200 bg-white shadow-sm">
        {loading ? (
          ...
          <div className="py-20 text-center">
            ...
            <p className="mt-1 text-sm text-gray-400">Sessions will appear here once users connect to a server.</p>
          </div>
        ) : (
          <>
            ...
            {totalPages > 1 && (
              <div className="flex items-center justify-between border-t border-gray-100 px-5 py-3">
                <p className="text-xs text-gray-500">{total} total session{total !== 1 ? 's' : ''}</p>
                <div className="flex items-center gap-2">
                  <button onClick={() => setPage((p) => Math.max(1, p - 1))} disabled={page === 1} className="flex h-7 w-7 items-center justify-center rounded border border-gray-200 text-gray-600 hover:bg-gray-50 disabled:opacity-40">
                    <ChevronLeft className="h-3.5 w-3.5" />
                  </button>
                  <span className="text-xs text-gray-600">Page {page} of {totalPages}</span>
                  <button onClick={() => setPage((p) => Math.min(totalPages, p + 1))} disabled={page === totalPages} className="flex h-7 w-7 items-center justify-center rounded border border-gray-200 text-gray-600 hover:bg-gray-50 disabled:opacity-40">
