...
function ServiceCard({
  ...
  const href = getServiceLaunchHref(serviceKey, 'tenant', projectId);
  const transactionsHref = getServiceTransactionsHref(serviceKey, 'tenant', projectId);
...
export default function TenantProjectDetailPage() {
  ...
  async function handleSave(e: React.FormEvent) {
    ...
    setFlash(null);
    try {
      const updated = await updateTenantProject(project.id, {
        name: name.trim(),
  ...
  async function handleAddService(key: AdminServiceKey) {
    ...
    setError(null);
    try {
      const updated = await addTenantProjectServices(project.id, [key]);
      setProject(updated);
  ...
  async function handleRemoveService(key: AdminServiceKey) {
    ...
    setError(null);
    try {
      const updated = await removeTenantProjectService(project.id, key);
      setProject(updated);
  ...
  if (!project) {
    return (
      <div className="space-y-4">
        ...
        <button
          type="button"
          onClick={() => router.push(tenantConsole.projects)}
  ...
  return (
    <div className="mx-auto max-w-screen-xl space-y-8 pb-10">
      ...
      <section>
        <h2 className="mb-4 text-base font-semibold text-gray-900">Project settings</h2>
        <form
          onSubmit={handleSave}
          ...
          <div className="flex items-center justify-between">
            {!archived ? (
              ...
              <span className="text-sm text-gray-400">
                Project is archived and cannot be edited.
              </span>
            )}
