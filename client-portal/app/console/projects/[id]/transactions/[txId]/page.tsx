...
import { getMyAdminWalletTransaction } from '@/lib/adminBillingApi';
...
export default function OrgProjectTransactionDetailPage() {
  ...
  const listHref = serviceKey
    ? `/console/projects/${projectId}/transactions?serviceKey=${encodeURIComponent(serviceKey)}`
  ...
  const load = useCallback(async () => {
    ...
    try {
      const [project, transaction] = await Promise.all([
        ...
        getMyAdminWalletTransaction(txId),
      ]);
      ...
      setTx({
        ...
        relatedRef: transaction.relatedVmJobId,
  ...
  return (
    <ProjectTransactionDetailView
      ...
      backHref={listHref}
      listHref={`/console/projects/${projectId}/transactions`}
      tx={tx}
    />
  );
}
