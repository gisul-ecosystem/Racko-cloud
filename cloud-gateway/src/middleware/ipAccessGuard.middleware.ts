...
function expandIPv6(ip: string): number[] | null {
  ...
  // Has :: — expand missing groups as zeros
...
function isIpAllowed(clientIp: string, allowedIps: string[]): boolean {
  ...
  for (const entry of allowedIps) {
    ...
    const entryHost = e.replace(/\/\d+$/, '');
    const entryIsV6 = isIPv6(entryHost);
 ...
 *
 * Fall back to req.ip only when the header is absent (direct connections).
 */
 ...
 * Supports IPv4, IPv4 CIDR, IPv6 exact, and IPv6 CIDR entries.
 * All other requests (no tenant context, or mode = 'all') pass unchanged.
 *
 * Never blocks in development mode.
 */
