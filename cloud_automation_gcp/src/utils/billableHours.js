...
export function normalizeUsageWindowsForBilling(usageWindows) {
  if (!Array.isArray(usageWindows)) return [];

  return usageWindows
    .map((window) => ({
      day_of_week: Number(window?.day_of_week ?? window?.dayOfWeek),
      window_start_time: String(window?.window_start_time ?? window?.windowStartTime ?? '').trim(),
      window_end_time: String(window?.window_end_time ?? window?.windowEndTime ?? '').trim(),
      timezone: String(window?.timezone || '').trim() || 'Asia/Kolkata',
      daily_limit_hours:
        window?.daily_limit_hours != null
          ? Number(window.daily_limit_hours)
          : window?.dailyLimitHours != null
            ? Number(window.dailyLimitHours)
            : null,
    }))
    .filter(
      (window) =>
...
function computeWindowHoursForDay(window) {
...
export function computeBillableHours(startDate, endDate, usageWindows = []) {
  ...
  if (!start.isValid || !end.isValid || end < start) {
    return { calendarHours: 1, billableHours: 1, usesUsageWindows: false };
  }

  const calendarHours = Math.max(1, end.diff(start, 'hours').hours);
  const normalizedWindows = normalizeUsageWindowsForBilling(usageWindows);
  ...
  const windowsByDay = new Map(normalizedWindows.map((window) => [window.day_of_week, window]));
