  ...
  return usageWindows
    .map((window) => ({
      ...
      window_start_time: String(
        window?.window_start_time ?? window?.windowStartTime ?? ''
      ).trim(),
...
/**
 * Azure-parity billable hours:
 * - no usage windows → calendar hours between start/end (min 1)
 * - with usage windows → overlapping window hours only (min 1)
 */
