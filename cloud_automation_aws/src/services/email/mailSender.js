...
async function sendViaZoho({ to, subject, html, text, attachments = [] }) {
  const config = getEmailConfig();
...
export async function sendMailWithRetry({ to, subject, html, text, attachments = [] }) {
  ...
  const config = getEmailConfig();
  const sendOnce = config.provider === 'zoho_zeptomail' ? sendViaZoho : sendViaResend;
